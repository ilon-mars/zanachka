...
import { EASY_PERCENTS } from '@/utils/constants';

export const useEasyStore = defineStore('easy', () => {
  /* init localStorage */
  const localData: FixedEnvelopesModel = getFromLS(MethodEnum.EASY);

  /* component logic */
  const income = ref<string>(localData?.income.toString() || '');
  const fieldsList = computed(() =>
    EASY_PERCENTS.map((percent, index) => ({
  ...
  const save = () => {
    ...
    saveToLS(MethodEnum.EASY, budget);
  };

  const clear = () => {
    income.value = '';

    removeFromLS(MethodEnum.EASY);
  };
